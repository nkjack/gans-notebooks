...
class Generator(nn.Module):
    def __init__(self, z_dim, featuremap_size=4, out_channels=3):
        ...
        all_modules = [
            nn.ConvTranspose2d(self.z_dim, 1024, featuremap_size, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(1024),
            nn.ReLU(),
...
def discriminator_loss_fn(y_data, y_generated, data_label=0, label_noise=0.0):
    ...
    n_y_data =  torch.distributions.uniform.Uniform(-label_noise / 2, label_noise / 2).sample((b_size,)).to(device) + data_label
    n_y_gen = torch.distributions.uniform.Uniform(-label_noise / 2, label_noise / 2).sample((b_size,)).to(device) + (1 - data_label)
    
    # loss_fn = nn.BCEWithLogitsLoss()
    loss_fn = nn.BCELoss()
    loss_data = loss_fn(y_data.view(-1), n_y_data)
    loss_generated = loss_fn(y_generated.view(-1), n_y_gen)
    # ========================
...
def generator_loss_fn(y_generated, data_label=0):
    ...
    # loss_fn = nn.BCEWithLogitsLoss()
    loss_fn = nn.BCELoss()
    loss = loss_fn(y_generated.view(-1), data_label_tensor.view(-1))
    # ========================
    return loss
